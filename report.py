    ...
    if what == 'agent':
        ...
        for c in clients:
            ...
            if oldest_date == 0 or oldest_date > dateint:
                oldest_date = dateint
                oldest_i = c[0]
        old_obj = execute_single_row_query("SELECT h.id,m.mac_addr,ip.ipaddr,h.name FROM hosts h INNER JOIN macs m ON h.mac_id=m.id INNER JOIN ipaddrs ip ON h.ipaddr_id=ip.id WHERE h.id='{0}'".format(oldest_i))
    else:
    ...
    if what == 'agent':
        ...
        for c in clients:
            ...
            if newest_date == 0 or newest_date < dateint:
                newest_date = dateint
                newest_i = c[0]
        new_obj = execute_single_row_query("SELECT h.id,m.mac_addr,ip.ipaddr,h.name FROM hosts h INNER JOIN macs m ON h.mac_id=m.id INNER JOIN ipaddrs ip ON h.ipaddr_id=ip.id WHERE h.id='{0}'".format(newest_i))
    else:
        raise Exception("Unrecognized entity ({0})".format(what))
...
def main():
    ...
    if args.report_type == 'agent-summary':
        ...
        clients = execute_multi_row_query("SELECT id FROM hosts ORDER BY name")
        print "=" * 105
        print "| %-17s | %-17s | %-17s | %-19s | %-19s |" % ("Name","MAC Addr","IP Addr","Date Discovered","Last Updated")
        for hid in clients:
            #pp.pprint(hid)
            ctup = execute_single_row_query("SELECT h.name,m.mac_addr,ip.ipaddr,h.date_discovered,h.last_updated FROM hosts h INNER JOIN macs m ON h.mac_id=m.id INNER JOIN ipaddrs ip ON h.ipaddr_id=ip.id WHERE h.id='{0}'".format(hid[0]))
            #pp.pprint(ctup)
